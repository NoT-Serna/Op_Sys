...
public class PMM_Segmentation extends ProcessMemoryManager{
    ...
    MemoryAddress segment_address = getSegmentMemoryAddressFromLocalAddress(logicalAddress);
    MemoryAddress phys_address = getPhysicalMemoryAddressFromLogicalMemoryAddress(segment_address);
    if (phys_address != null) {
      
        return phys_address.getDivision() + phys_address.getOffset();
    } else {
        return -1;
        }
    }
